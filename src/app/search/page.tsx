...
export default function SearchPage() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile) {
      ...
      try {
        const response = await fetch('https://10c4-35-204-196-244.ngrok-free.app/upload-file', {
          method: 'POST',
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch("https://10c4-35-204-196-244.ngrok-free.app/query", {
        method: "POST",
        headers: {
